...
function inicializarDados() {
    let dados = JSON.parse(localStorage.getItem("gmflix-users"));
    if (dados === null) {
        dados = [
            {
                nome: "Gabriel",
        ...
        localStorage.setItem("gmflix-users", JSON.stringify(dados));
    }
    perfis = dados;
}

// ao carregar a página executa as funções de busca de dados
...
function adicionarPerfil(){
    ...
    perfis.push(perfil);
    localStorage.setItem("gmflix-users", JSON.stringify(perfis));
...
function countText() {
    let charCount = document.getElementById('gameId').value.lenght;
